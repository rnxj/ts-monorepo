...
const client = testClient<AppType>(createApp().route("/", router));
...
describe("tasks routes", () => {
  ...
  it("post /tasks validates the body when creating", async () => {
    const response = await client.tasks.$post({
      // @ts-expect-error - Testing validation: missing required 'name' field
      json: {
        done: false,
      },
    });
  ...
  it("get /tasks/{id} validates the id param", async () => {
    const response = await client.tasks[":id"].$get({
      param: {
        // @ts-expect-error - Testing validation: expected number, received string
        id: "wat",
      },
    });
